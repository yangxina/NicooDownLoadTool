...
public class NicooManager {
    ...
    public static var logLevel: NicooLogLevel = .none

    public static var isControlNetworkActivityIndicator = true

    private let queue: DispatchQueue = DispatchQueue(label: "com.Nicoo.queue")

    public var cache: NicooCache
    ...
    public internal(set) var status: NicooStatus {
        get {
            return queue.sync {
                internalStatus
            }
    ...
    private var internalTasks: [NicooTask] = []
    public internal(set) var tasks: [NicooTask] {
        get {
            return queue.sync {
                internalTasks
            }
    ...
    public var runningTasks: [NicooTask] {
        return tasks.filter { $0.status == .running }
    }

    public var completedTasks: [NicooTask] {
        return tasks.filter { $0.status == .completed }
    }
    
    public var unCompletedTasks: [NicooTask] {
        return tasks.filter { $0.status != .completed }
    }
    ...
    internal var successHandler: NicooManagerHandler?
    
    internal var failureHandler: NicooManagerHandler?
    
    internal var progressHandler: NicooManagerHandler?
    ...
    public init(_ name: String? = nil, MaximumRunning: Int? = nil, isStoreInfo: Bool = false) {
        if name != nil {
            cache = NicooCache(name!)
        } else {
            cache = NicooCache.default
        }
        ...
        let sessionDelegate = NicooSessionDelegate()
        ...
        if isStoreInfo {
            ...
            NicooLog("retrieveTasks tasks.count = \(tasks.count)")
...
extension NicooManager {
    ...
    @discardableResult
    public func download(_ URLString: String, fileName: String? = nil, progressHandler: NicooTaskHandler? = nil, successHandler: NicooTaskHandler? = nil, failureHandler: NicooTaskHandler? = nil) -> NicooDownloadTask? {
        status = .waiting

        guard let url = URL(string: URLString) else {
            NicooLog("[manager] 下载无效的URLString：\(URLString)")
            return nil
        }

        isCompleted = false

        var task = fetchTask(URLString) as? NicooDownloadTask
        if task != nil {
            ...
            task = NicooDownloadTask(url, fileName: fileName, cache: cache, progressHandler: progressHandler, successHandler: successHandler, failureHandler: failureHandler)
            tasks.append(task!)
        }
    ...
    @discardableResult
    public func multiDownload(_ URLStrings: [String], fileNames: [String]? = nil, progressHandler: NicooTaskHandler? = nil, successHandler: NicooTaskHandler? = nil, failureHandler: NicooTaskHandler? = nil) -> [NicooDownloadTask] {
        status = .waiting
        ...
        if uniqueUrls.isEmpty {
            return [NicooDownloadTask]()
        }

        isCompleted = false

        var temp = [NicooDownloadTask]()
        for url in uniqueUrls {

            var task = fetchTask(url.absoluteString) as? NicooDownloadTask
            if task != nil {
                task!.progressHandler = progressHandler
                task!.successHandler = successHandler
                task!.failureHandler = failureHandler
...
extension NicooManager {
    ...
    public func fetchTask(_ URLString: String) -> NicooTask? {
    ...
    public func start(_ URLString: String) {
        guard let task = fetchTask(URLString) as? NicooDownloadTask else { return }
        task.manager = self
        task.session = session

        if cache.fileExists(fileName: task.fileName) {
            NicooLog("[manager] file is exists URLString: \(task.URLString)")
            NicooLog("[manager] file is exists URLString: \(cache.filePtah(fileName: task.fileName)!)")
        ...
        case .completed:
            NicooLog("[manager] task has completed URLString: \(URLString)")
            task.completed()
            self.completed()
        case .running:
            NicooLog("[manager] task is running URLString: \(URLString)")
        default: break
        }
    }
...
extension NicooManager {
    ...
    internal func completed() {
        ...
        if status == .preRemove {
            if tasks.isEmpty {
                status = .remove
                NicooLog("[manager] all tasks completed and manager failed because its status is \(status)")
                DispatchQueue.main.tr.safeAsync {
        ...
        if isEnd {
            ...
            let isSuccess = tasks.filter { $0.status == .failed }.isEmpty
            if isSuccess {
                NicooLog("[manager] all tasks completed and manager succeeded")
                status = .completed
                DispatchQueue.main.tr.safeAsync {
                    self.successHandler?(self)
                }
            } else {
                NicooLog("[manager] all tasks completed and manager failed")
                status = .failed
                DispatchQueue.main.tr.safeAsync {
                    self.failureHandler?(self)
                }
        ...
        if isSuspend {
            ...
            self.isSuspend = true
            NicooLog("[manager] manager did suspend")
            status = .suspend
            DispatchQueue.main.tr.safeAsync {
                self.successHandler?(self)
            }
        ...
        NicooLog("[manager] start to download next task")
    ...
    internal func parseSpeed() {
        ...
        if cost <= 0.8 {
            if speed == 0 {
                ...
                tasks.forEach({ (task) in
                    if let task = task as? NicooDownloadTask {
...
extension NicooManager {
    @discardableResult
    public func progress(_ handler: @escaping NicooManagerHandler) -> Self {
        progressHandler = handler
        return self
    }
    
    @discardableResult
    public func success(_ handler: @escaping NicooManagerHandler) -> Self {
        successHandler = handler
        return self
    }
    
    @discardableResult
    public func failure(_ handler: @escaping NicooManagerHandler) -> Self {
